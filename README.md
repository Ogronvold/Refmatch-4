# RefMatch 0.5.65 — Matched label cleanup

- Completed Match button now displays `MATCHED` with no checkmark.
- This removes the encoding issue that previously rendered an extra malformed character.
- Plugin/CMake version: 0.5.65.
- All other behavior remains unchanged from the previous build.

See `BUILD-NOTES.md` for implementation history and `RELEASE-SETUP.md` for release preparation.
