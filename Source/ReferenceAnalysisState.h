...
public:
    ...
    LoudnessTracker loudness;
    static constexpr double sampleRate=48000.;
    ReferenceAnalysisState() { loudness.prepare(sampleRate, 2); }
    void acceptAudio(const juce::AudioBuffer<float>& audio) {
        present.store(true);analyser.pushBlock(audio);learning.push(audio,audio,true,sampleRate);loudness.processBlock(audio);
